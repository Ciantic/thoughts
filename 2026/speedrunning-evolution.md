...
This post is about Robert Wright's latest book [The God Test: Artificial Intelligence and Our Coming Cosmic Reckoning](https://thegodtest.net/). He is known for evolutionary psychology among other things, and while the book covers a lot more than similarities with LLMs and our brains, this post is only about that part. He gives three examples of the similarities. The first is the meaning of words, which is evidently important for mastering language. The second is an example from the visual cortex, where we are known to have specific neurons for detecting edges. The third one is a bit harder to identify: theory of mind.

I find the visual cortex example most compelling, because we can observe via imaging that human brains have developed similar functionality, whereas the conceptual examples of the meaning of words, and theory of mind are harder to verify. I will go through them the way they are introduced in the book.

Meaning of words can be seen as a multidimensional semantic space. One dimension, as Robert Wright illustrates, could be lethality, one could be speed, or size and so on. There can be an innumerable number of dimensions, and LLMs during their training construct this kind of mapping. Nobody has given LLMs a task to figure out the meaning of words, but it arises out of the task of predicting the next word.
...
It is much harder to say whether our brains use similar mapping as LLMs. To me it seems plausible, given that it could also be the most efficient way to compress the information by colocating and connecting concepts with similar meaning. Interestingly we know where the meaning of words is stored in the brain. In humans [Wernicke's area](https://en.wikipedia.org/wiki/Wernicke%27s_area) is responsible for assigning meaning to words, and a certain kind of brain damage can cause a human to talk nonsense in a syntactically sensible way. Language is of course much more than the meaning of words, but it is harder to pinpoint the rest of the functionality.
...
What makes this example compelling is the evidence. You show people pictures of angled lines and see what parts of the brain "light up". Given that different angled lines have distinct neurons, we can conclude that indeed similar functionality must exist. It is likely also the case that this is the obvious solution for image recognition in neural networks, so both natural selection and LLMs discovered it independently. We developed edge detection neurons via evolution's trial and error during our path from a single-celled ocelloid to the human eye, while LLMs discovered it by training on vast datasets.

Third example is [theory of mind](https://en.wikipedia.org/wiki/Theory_of_mind), one's ability to attribute different mental states to someone else. It appears that theory of mind formed in ChatGPT 4 for the first time:
...
Robert Wright also came up with his own test, which he tested on ChatGPT 4:
...
You don't need to grasp a lot of information theory to know that LLMs can't have all the answers in their "corpus" as some seem to assume. It just wouldn't fit, there is no way to make a database to autocomplete all the ways you can question them. LLMs must have true understanding of language to make sensible answers. They do make mistakes, but so do humans. However, LLMs haven't yet been trained with truly multi-modal datasets. They don't know what it is like to drive a car to a carwash, and may never know what food tastes like.[^1]

What we train LLMs on is largely driven by commercial needs, the ability to understand the world at the level of language is in much higher demand than creating artificial taste buds. There are a lot of low-hanging fruits in the commercial space for LLM training, one obvious one being robotics and spatial understanding. I tried to query ChatGPT about measurements from a picture, and it struggled. It got confused by barrel distortion, subsequently hallucinated the size of a fire alarm, and had no ability to reason about the object dimensions along the perspective line. After all, they aren't yet trained to understand "can I reach that object", or "how should I maneuver my body to reach that object". They don't know yet how far things are or how to get there.

This is from the beginning of the book, but it fits nicely at the end of this post:
...
[^1]: With food, there are more complications; presumably the taste of food changes based on your nutrient levels and how sated you feel. How would we even approach making a good enough dataset for that? LLMs might feel satisfied from the good sip of electricity though.